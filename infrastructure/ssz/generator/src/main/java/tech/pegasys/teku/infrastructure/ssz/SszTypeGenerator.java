...
public class SszTypeGenerator<T extends SszContainer, S extends SszSchema<T>> {
  ...
  public S defineType()
      throws NotFoundException, CannotCompileException, InvocationTargetException,
  ...
  private void defineTypeMethods(final CtClass implClass)
    ...
    for (int i = 0; i < fields.length; i++) {
      ...
      final String methodName = "get" + CaseUtils.toCamelCase(field.getName(), true);
      final Method declaredMethod = typeInterface.getDeclaredMethod(methodName);
      // TODO: Should validate that the return type actually matches the field type
      addMethod(
          implClass,
          ""
              + "public %s %s() { \n"
  ...
  private void defineSchemaMethods(final CtClass schemaClass)
    ...
    // Define teh createFromBackingNode abstract method to call the type class's constructor
    addMethod(
        schemaClass,
        ""
            + "public %s createFromBackingNode(tech.pegasys.teku.infrastructure.ssz.tree.TreeNode node) { \n"
    ...
    // TODO: Need to verify the params in the create method actually match up to the required types
    ...
    // For each field, define a getter for it's schema.
    for (int i = 0; i < fields.length; i++) {
      final NamedSchema<?> field = fields[i];
      final String methodName = "get" + CaseUtils.toCamelCase(field.getName(), true) + "Schema";
      final Method declaredMethod = schemaInterface.getDeclaredMethod(methodName);
      // TODO: Should validate that the return type actually matches the schema type
      addMethod(
          schemaClass,
          ""
              + "public %s %s() { \n"
  ...
  private S createSchema(final CtClass schemaClass)
      throws CannotCompileException, NoSuchMethodException, InvocationTargetException,
  ...
  private void addConstructor(final CtClass ctClass, final String method, final Object... args)
    ...
    ctClass.addConstructor(CtNewConstructor.make(String.format(method, args), ctClass));
  }
